...
router.get('/connection_list', function (req, res, next) {
    var connection_list = req.nconf.connections.get('connections');
		var out = { message: "", connection_list: order_object(connection_list) }

		if (isModeApi){
			res.json(out);
		}
		else{
			res.render('connections', out);
		}
});
